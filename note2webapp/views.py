    ...
    if page == "create":
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if ModelUpload.objects.filter(user=request.user, name=model_name).exists():
                    messages.error(request, f"A model with the name '{model_name}' already exists. Please choose a different name.")
                    context["form"] = form
    ...
    # Add version
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(request, f"Version '{version.tag}' uploaded successfully!")
                return redirect(f"/dashboard/?page=detail&pk={upload.pk}")
        else:
            form = VersionForm()
        context["form"] = form
...
def soft_delete_version(request, version_id):
    ...
    if version.is_active:
        ...
        if other_versions.exists():
            ...
            messages.error(request, "Cannot delete active version. Please activate another version first.")
            return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        files_to_delete = [version.model_file, version.predict_file, version.schema_file]
        ...
        messages.success(request, f"Version with tag '{version.tag}' has been deleted successfully.")
...
def activate_version(request, version_id):
    ...
    if version.is_deleted:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'success': False, 'error': 'Cannot activate deleted version'}, status=400)
...
def delete_model(request, model_id):
    ...
    active_version_count = ModelVersion.objects.filter(upload=model_upload, is_deleted=False).count()
    ...
    if active_version_count > 0:
        ...
        messages.error(request, "Cannot delete model with active versions. Please delete all versions first.")
        return redirect('dashboard')
...
def deprecate_version(request, version_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f"Version with tag '{version.tag}' has been deprecated.")
        return redirect(f'/model-versions/{version.upload.id}/')
